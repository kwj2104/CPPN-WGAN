...
def generate_image(frame, netG):
    ...
    ones = torch.ones(BATCH_SIZE, IMAGE_SIZE * IMAGE_SIZE, 1)
    if use_cuda:
        ones = ones.cuda()
    ...
    lib.save_images.save_images(
        samples,
        'tmp/casia/samples_{}.png'.format(frame)
    )
    ...
    for iteration in range(ITERS):
        ...
        for iter_d in range(CRITIC_ITERS):
            ...
            if real_data.size()[0] == BATCH_SIZE:
                ...
                real_data_v = autograd.Variable(real_data.view(BATCH_SIZE, -1))
        ...
        lib.plot.plot('tmp/casia/time', time.time() - start_time)
        lib.plot.plot('tmp/casia/train disc cost', D_cost.cpu().data.numpy())
        lib.plot.plot('tmp/casia/train gen cost', G_cost.cpu().data.numpy())
        lib.plot.plot('tmp/casia/wasserstein distance', Wasserstein_D.cpu().data.numpy())
        ...
        if iteration % 20000 == 0:
            torch.save(netG.state_dict(), "tmp/casia/G-cppn-wgan_{}.pth".format(iteration))
            torch.save(netD.state_dict(), "tmp/casia/D-cppn-wgan_{}.pth".format(iteration))
